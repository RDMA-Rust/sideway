...
impl BasicWorkCompletion<'_> {
    ...
    /// Vendor specific error which provides more information if the completion ended with error.
    pub fn vendor_err(&self) -> u32 {
        self.wc.vendor_err
    }

    /// The number of bytes transferred, relevant if the receive queue for incoming Send or RDMA
