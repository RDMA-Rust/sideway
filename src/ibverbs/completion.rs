...
impl BasicCompletionQueue {
    ...
    pub fn iter(&self) -> Result<BasicCompletionQueueIter<'_>, PollCompletionQueueError> {
        BasicCompletionQueueIter::new(self.cq, unsafe { self.poll_batch.get().try_into().unwrap_unchecked() })
    }
...
impl<'cq> Iterator for BasicCompletionQueueIter<'cq> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        match ret {
            ...
            _ => {
                // Either there is an error from ibv_poll_cq or we are done, so we stop iterating.
                self.status = Empty;
                None
            }
            ,
