...
impl Event {
    ...
    pub fn cm_id(&self) -> Option<Arc<CommunicationManager>> {
        unsafe {
            let raw_cm_id = self.event.as_ref().id;

            if !raw_cm_id.is_null() {
                if self.event_type() == EventType::ConnectRequest {
                    ...
                    (*raw_cm_id).context = Arc::into_raw(cm.clone()) as *mut std::ffi::c_void;

                    return Some(cm);
                }

                // For other events, return the existing CommunicationManager
                let context_ptr = (*raw_cm_id).context;
    ...
    pub fn listener_id(&self) -> Option<Arc<CommunicationManager>> {
        unsafe {
            let raw_listen_id = self.event.as_ref().listen_id;
    ...
    pub fn create_id(&mut self, port_space: PortSpace) -> Result<Arc<CommunicationManager>, String> {
        ...
        let ret = unsafe {
            rdma_create_id(
                ...
                Arc::into_raw(cm.clone()) as *mut std::ffi::c_void,
                port_space as u32,
            )
        };
...
impl CommunicationManager {
    ...
    pub fn resolve_addr(&self, src_addr: SocketAddr, dst_addr: SocketAddr, timeout_ms: i32) -> Result<(), String> {
        let cm_id = unsafe { &mut *self.cm_id.get() };
        let mut src_addr = OsSocketAddr::from(src_addr);
        let mut dst_addr = OsSocketAddr::from(dst_addr);

        let ret =
            unsafe { rdma_resolve_addr(cm_id.as_mut(), src_addr.as_mut_ptr(), dst_addr.as_mut_ptr(), timeout_ms) };
