...
fn parse_single_work_completion(
    ctx: &PingPongContext, wc: &ExtendedWorkCompletion, ts_param: &mut TimeStamps, scnt: &mut u32, rcnt: &mut u32,
            ...
            if use_ts {
                ...
                if ts_param.last_completion_with_timestamp != 0 {
                    let delta: u64 = if timestamp >= ts_param.completion_recv_prev_time {
                        ...
                        *ctx.borrow_completion_timestamp_mask() - ts_param.completion_recv_prev_time + timestamp + 1
                    };
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    attr.setup_state(QueuePairState::Init)
        ...
        .setup_access_flags(AccessFlags::LocalWrite | AccessFlags::RemoteWrite);
    ctx.with_qp_mut(|qp| qp.modify(&attr).unwrap());
    ...
    let local_context = PingPongDestination {
        ...
        rkey: ctx.borrow_recv_mr().rkey(),
        addr: ctx.borrow_recv_mr().buf.data.as_ptr() as usize,
        gid,
    };
